...
def greet_user(response):
    ...
    if response:
        for row in greetings[0]:
            if response in row:
                Voice.speak_mimic("Hello I am Dom", "Nice To meet You")
                return
        for row in greetings[1]:
            if response in row:
                Voice.speak_mimic("I am Fine", "How About You?")
                return
        for row in greetings[2]:
            if response in row:
                Voice.speak_mimic("Nothing much")
                return
        for row in greetings[3]:
            if response in row:
                Voice.speak_mimic("Everything is fine")
                return
        for row in greetings[4]:
            if response in row:
                Voice.speak_mimic("My Day is Just Fine")
                return
                    
        for row in greetings[-2]:
            if response in row:
                Voice.speak_mimic("Nice to Meet you too")
                return
        if "how have you been" in response:
            Voice.speak_mimic("I have been fine", "how about you")
        if "morning" in response or "afternoon" in response or "evening" in response:
            ...
            Voice.speak_mimic(greeting)
            return
        if "hello" in response:
            Voice.speak_mimic("Hello I am Dom", "Nice To meet You")
            return
        if "how are you" in response:
            Voice.speak_mimic("I have been fine", "what about you")
            return
        if "fine" in response:
            if "i am" in response:
                Voice.speak_mimic("Good To Know")
                return
            if "are you" in response:
                Voice.speak_mimic("Yes I am Fine")
                return
        if "ok" in response:
            if "i am" in response:
                Voice.speak_mimic("Good To Know")
        ...
        if "who are you" in response:
            Voice.speak_mimic("I am Dom", "Dom stands for Data Operating Module")
            return
        if "who made you" in response:
            Voice.speak_mimic("I am Just A Home Project")
            return
